...
<form:form method="post" id="configureForm" class="configure_form" action="${configureProductUrl}">
    ...
    <c:if test="${empty showAddToCart ? true : showAddToCart}">
        ...
        <c:choose>
            <c:when test="${fn:contains(buttonType, 'button')}">
                <c:if test="${product.configurable}">
                    <button id="configureProduct" type="button"
                            class="btn btn-primary btn-block js-enable-btn outOfStock" disabled="disabled">
            ...
            <c:otherwise>
                <c:if test="${product.configurable}">
                    <button id="configureProduct" type="${buttonType}" class="btn btn-primary btn-block js-enable-btn"
                            disabled="disabled"
                            name="configure">
                        <spring:theme code="basket.configure.product"/>
                    </button>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <button id="addToCartButton" type="${buttonType}"
                                    class="btn btn-block btn-brand js-add-to-cart js-enable-btn"
                                    disabled="disabled">
